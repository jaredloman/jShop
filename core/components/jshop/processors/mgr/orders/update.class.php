...
*/
class OrderUpdateProcessor extends modObjectUpdateProcessor {
    public $classKey = 'jsOrder';
    public $languageTopics = array('jshop:default');
    public $objectType = 'js.order';
	
	public function afterSave() {
		$sendStatus = $this->getProperty('sendStatus');
		if ($sendStatus == true) {
			// Get Properties
			$oid = $this->getProperty('id');
			$status = $this->getProperty('status');
			$tpl = $this->modx->getOption('js.statusUpdateTpl',$scriptProperties,'statusUpdateTpl');
			$subj = $this->modx->getOption('js.statusSubect',$scriptProperties,'Order Status Update');
			
			$this->modx->js->emailStatus($oid,$status,$subj,$tpl);
		}
	}
}
