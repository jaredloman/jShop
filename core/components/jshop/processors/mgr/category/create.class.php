...
class CategoryCreateProcessor extends modObjectCreateProcessor {
    public $classKey = 'jsCategory';
    public $languageTopics = array('jshop:default');
    public $objectType = 'js.category';
 
    public function beforeSave() {
        $name = $this->getProperty('name');
 
        if (empty($name)) {
            $this->addFieldError('name',$this->modx->lexicon('js.category_err_ns_name'));
        } else if ($this->doesAlreadyExist(array('name' => $name))) {
            $this->addFieldError('name',$this->modx->lexicon('js.category_err_ae'));
        }
