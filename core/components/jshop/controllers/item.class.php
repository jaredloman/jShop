...
*/
class jShopItemManagerController extends jShopManagerController {
    public function process(array $scriptProperties = array()) {
 
    }
    public function getPageTitle() { return $this->modx->lexicon('jshop'); }
    public function loadCustomCssJs() {
		$this->addJavascript($this->js->config['jsUrl'].'mgr/widgets/item/numeric.input.js');
		$this->addJavascript($this->js->config['jsUrl'].'mgr/widgets/item/images.grid.js');
		$this->addJavascript($this->js->config['jsUrl'].'mgr/widgets/item/options.grid.js');
		$this->addJavascript($this->js->config['jsUrl'].'mgr/widgets/item/item.panel.js');
        $this->addLastJavascript($this->js->config['jsUrl'].'mgr/sections/item.js');
    }
    public function getTemplateFile() { return $this->js->config['templatesPath'].'item.tpl'; }
